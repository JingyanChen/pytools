                    ...
                    if match:
                        ...
                        if beginInfoName_match:
                            ...
                            if render_pass_match and framebuffer_match:
                                ...
                                #found endrenderpass
                                for j, line in enumerate(script_code.split('\n'), start = i):
                                    endRenderpassSuccess = 0
                                    if "vkCmdEndRenderPass(" + info.cb in line:
                                        print(line,j)
                                        end_renderpass_lines.append(j)
                                        ...
                                        rb_definition = re.search(rf'vkCreateRenderPass\((.*?), (.*?), (.*?), {re.escape(escaped_info_rp)}\);', script_code)
                                        #print(f'vkCreateRenderPass\d+\((.*?), (.*?), (.*?), {escaped_info_rp}\);')
                                        ...
                                        info = renderpassInfo()
                                        endRenderpassSuccess = 1
                                        break
                                if(endRenderpassSuccess == 0):
                                    print(f"endrenderpass scan failed! begin line {i}: renderpass:{info.rb} beginRenderpass: {info.beginLine} command buffer: {info.cb}")
